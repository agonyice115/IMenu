...
public class DynamicParser extends BaseParser<Map<String, Object>> {
	...
	public DynamicParser(int TimeType) {
	...
	public Map<String, Object> parseData(JSONObject jsonObject)
		...
		for (int i = 0; i < jsonDataArray.length(); i++) {
			...
			/** 动态的个人信息数据 */
			if (object.optJSONObject("view_member") != null) {
				...
				if (TimeType == WEEK) {
					for (int k = 0; k < dynamicList.size(); k++) {
						...
						if (dynamicDate.equals(key.substring(key.length() - 2,
								key.length()))) {
							System.out.println("filterDate.key:"
									+ filterDate.key);
							newDynamicsList.add(dynamicList.get(k));
						}
					}
